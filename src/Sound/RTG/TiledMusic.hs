...
resync s (Tile pre post m) = let npost = post + s
                             in if npost < 0 then Tile (pre - npost) 0 (mDelay (-npost) m)
...
coresync s (Tile pre post m) = let npre = pre + s
                               in if npre < 0  then Tile 0 (post - npre) (mDelay (-npre) m)
