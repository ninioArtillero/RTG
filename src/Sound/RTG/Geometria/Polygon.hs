...
-- and displacement of two coprime polygons.
...
perfectlyBalancedRhythms n =
  let availablePolygons = map (basePolygon n) (divisors n)
      allPolygonPositions = concatMap polygonPositionList availablePolygons
      allUnityWeightedPolygons = concatMap (\p -> [(1,p),(-1,p)]) allPolygonPositions
      patternList = map wPolygonPattern allUnityWeightedPolygons :: [[Int]]
      combinations = filter ((>= 2) . length) . subsequences $ patternList
      patternSum = zipWith (+)
      summedCombinations = map (foldl1 patternSum) combinations
  in braceletNub . filter ((`Set.isSubsetOf` Set.fromList [0,1]) . Set.fromList) $ summedCombinations
