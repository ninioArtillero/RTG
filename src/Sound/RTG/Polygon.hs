...
uniqueFactorisationList = concatMap unfold . Primes.factorise
  where
    unfold (p, q) = replicate (fromIntegral q) (Primes.unPrime p)
...
necklaceNub' = foldr (\x acc -> x : filterEquiv x acc) []
  where
    filterEquiv x = filter (not . equivNecklace x)
...
braceletNub' = foldr (\x acc -> x : filterEquiv x acc) []
  where
    filterEquiv x = filter (not . equivBracelet x)
